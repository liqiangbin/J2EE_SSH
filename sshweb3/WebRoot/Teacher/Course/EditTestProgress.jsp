...
<body>
   <table class="table table-bordered table-hover definewidth m10">
	<caption align="absmiddle"><h2>待填写实验进度表信息</h2></caption>
    <thead>
    <tr>
        <th>课程号</th>
        <th>课程名称</th>
     ...
     <% List<Sortprogress> list=(List<Sortprogress>)session.getAttribute("nofilltest"); %>
...
<form action="opentestword" method="post">
<!-- 模态框（Modal） -->
<div class="modal fade" id="<%=i %>" tabindex="-1" role="dialog" 
   ...
   <div class="modal-dialog">
	...
	String address="F:/myeclipse/myclipseworkspace/sshweb3/WebRoot/alltestprogress" + "/" +list.get(i).getCid()+list.get(i).getTid()+time+".docx";  %>
        ...
        <tr> <td width="20%">名称:</td>
         <td><input type="text" value="<%=name%>实验进度表"  name="testprogress.name"></td></tr>
         <tr>
          <tr> <td width="20%">教师名称:</td>
         <td><%=teacher.get(0).getName()%></td></tr>
         <input type="hidden" name="testprogress.tname" value="<%=teacher.get(0).getName()%>">
         <input type="hidden" name="testprogress.cid" value="<%=list.get(i).getCid()%>">
         <input type="hidden" name="testprogress.did" value="<%=teacher.get(0).getDid()%>">
         <input type="hidden" name="testprogress.tid" value="<%=teacher.get(0).getId()%>">
          <input type="hidden" name="testprogress.status" value="0">
          <input type="hidden" name="testprogress.address" value="<%=address%>">
         
         <tr>
         <td>选择学期：</td>
         <td>
         <select name="testprogress.term" class="form-control">
